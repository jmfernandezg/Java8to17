...
public class PredicateNot {
    ...
    private static final String FILE_PATH = "src/main/java/com/jmfg/training/java11/file1.txt";
    ...
    private static void nonBlankLinesLambda() throws IOException {
        ...
        try (Stream<String> lines = Files.lines(Paths.get(FILE_PATH))) {
            ...
            //Option one.
            List<String> nonBlankLines = lines.filter(line -> !line.isBlank()).collect(Collectors.toList());
    ...
    private static void nonBlankLinesMethodReference() throws IOException {
        ...
        try (var lines = Files.lines(Paths.get(FILE_PATH))) {
            //Option two
            List<String> nonBlankLines = lines.filter(((Predicate<String>) String::isBlank).negate()).collect(Collectors.toList());
            System.out.println(nonBlankLines);
        }
    }
    ...
    private static void nonBlankPredicateNot() throws IOException {
        ...
        try (var lines = Files.lines(Paths.get(FILE_PATH))) {
            ...
            List<String> nonBlankLines = lines.filter(Predicate.not(String::isBlank)).collect(Collectors.toList());
